# -----------------------------------------------------------------------------
# Added by liupenghui, based on Gmssl LIB, support SM4 CFB and OFB mode
# -----------------------------------------------------------------------------
...
class CryptSM4(object):
    ...
    #SM4-ECB buffer encryption/decryption
...
#SM4-OFB buffer encryption  
# M is type of bytes
def crypt_ofb_encrypt(key, iv, M):
    stream_key = []
    output_data = []  
    
    crypt_sm4 = CryptSM4()
    # setup the sm4 encryption key and padding the message
    crypt_sm4.set_key(key, SM4_ENCRYPT)
    M = bytes_to_list(M)
    M_value = padding(M)  

    # calculate the count of chain encryptions
    ...
    for i in range(stream_num):
        ...
        m = i * 16
        n = (i + 1) * 16
        output_data += xor(M_value[m:n], stream_key[i + 1])        

    return list_to_bytes(output_data)

#SM4-OFB buffer decryption   
# C is type of bytes    
def crypt_ofb_decrypt(key, iv, C):
    stream_key = []
    output_data = []    
    ...
    for i in range(stream_num):
        ...
        m = i * 16
        n = (i + 1) * 16
        output_data += xor(C_value[m:n], stream_key[i + 1])        

    output_data = unpadding(output_data)
    return list_to_bytes(output_data)

#SM4-CFB buffer encryption  
# M is type of bytes
def crypt_cfb_encrypt(key, iv, M):
    stream_key = []
    output_data = []  
    ...
    for i in range(stream_num):
        ...
        m = i * 16
        n = (i + 1) * 16        
        temp = xor(M_value[m:n], temp)
        output_data += temp
        stream_key.append(temp)

    return list_to_bytes(output_data)

#SM4-CFB buffer decryption  
# C is type of bytes    
def crypt_cfb_decrypt(key, iv, C):
    stream_key = []
    output_data = []    
    ...
    for i in range(stream_num):
        ...
        m = i * 16
        n = (i + 1) * 16
        temp = xor(C_value[m:n], temp)
        output_data += temp 
        stream_key.append(C_value[m:n])
