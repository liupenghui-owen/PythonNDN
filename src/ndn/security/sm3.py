...
def sm3_HAMC(key, msg=b""):
    if isinstance(key, memoryview):
        key = key.tobytes()
    try:
        if len(key) <= 32:
            key_0 = key + b"\x00" * (64 - len(key))
        else:
            hash_k = bytes.fromhex(sm3_hash(func.bytes_to_list(key)))
            key_0 = hash_k + b"\x00" * (64 - len(hash_k))
    except AttributeError:
        raise ValueError("Hash type incompatible to HMAC")
        
    key_0_ipad = strxor(key_0, b"\x36" * len(key_0))    
    hash_1 = bytes.fromhex(sm3_hash(func.bytes_to_list(key_0_ipad+msg)))
    key_0_opad = strxor(key_0, b"\x5c" * len(key_0))
    hash_2 = bytes.fromhex(sm3_hash(func.bytes_to_list(key_0_opad+hash_1)))
    return hash_2
