...
# Modified by liupenghui, added SignatureType SM3_WITH_SM2 to keep in line with NDNCXX
#
...
from .tlv_var import get_tl_num_size, write_tl_num
...
def get_data(name: NonStrictName,
              meta_info: MetaInfo,
              content: Optional[BinaryStr] = None,
              signer: Optional[Signer] = None) -> VarBinaryStr:
    r"""
    Make and get a Data packet.
    ...
    shrink_size = data.data._shrink_len.get_arg(markers['data##inner_markers'])
    #if shrink_size > 0:
    #    ret1 = shrink_length(ret, shrink_size)
    
    type_len = get_tl_num_size(TypeNumber.DATA)
    size_len = get_tl_num_size(len(ret) - shrink_size)
    buf = bytearray(type_len + size_len + len(ret) - shrink_size)
    write_tl_num(TypeNumber.DATA, buf)
    write_tl_num(len(ret) - shrink_size, buf, type_len)
    buf[type_len + size_len:] = memoryview(ret)[0:len(ret) - shrink_size]     
       
    if shrink_size > 0:       
        return buf, shrink_length(ret, shrink_size)
    else:
        return buf, ret    
